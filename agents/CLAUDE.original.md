...
10 modes via sub-skills (`sdlc:<mode>`): `full-ui`, `full-logic`, `lightweight-ui`, `lightweight-logic`, `refactor`, `hotfix`, `dependency-bump`, `config-data`, `docs-only`, `poc`.
`poc` is NOT shippable — requires `full-ui` or `full-logic` before merge to main.

### Orchestrator

Tasks w/ 3+ independent subtasks or multi-stage pipelines → Orchestrator spawns agents concurrently. `/sdlc` decides *what*; Orchestrator *runs it*.
