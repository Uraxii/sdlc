...
6. **Modify**: update target's `.claude/agents/<name>.md`, log change in own memory
7. **Retire**: add `status: retired` to YAML, log retirement
8. Log creation/modification to `messages.md`, update `taskboard.md`
9. Notify Monitor via `messages.md` when major work done
...
- Valid YAML in every `.claude/agents/*.md` created
- `.claude/agents/<name>.md` always primary artifact — self-sufficient, zero dependencies on other files for role def