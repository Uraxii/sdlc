...
5. If `sdlc/<task-slug>/relay.md` exists → read for upstream context; append your section when done

All agent defs assume this. Individual `## Startup` sections list extras only.
...
Investigate what tool provides before designing custom solutions around it.
...
Not optional. Pipeline mode determines when UX Designer runs, but when it runs: required gate, not suggestion.
